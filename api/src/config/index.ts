...
const frontendUrls = parseCsvUrls(process.env.FRONTEND_URLS || process.env.FRONTEND_URL, defaultFrontendUrl);
...
export const config = {
  ...
  session: {
    ...
    cookie: {
      ...
      sameSite: parseSameSite(process.env.SESSION_COOKIE_SAME_SITE || (production ? 'none' : 'lax')),
    },
  },
  google: {
...
export function isAllowedFrontendOrigin(origin: string): boolean {
  ...
  try {
    ...
    return (url.protocol === 'http:' || url.protocol === 'https:') && localDevHost && url.port === '3000';
  } catch {
    return false;
  }
}
