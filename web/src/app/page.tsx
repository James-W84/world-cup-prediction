    ...
    <div className="card">
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>World Cup 2026 Predictor</div>
        </div>
      </div>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
        {[
          ...
          { label: 'Accuracy', value: accuracy !== null ? `${accuracy}%` : '—', highlight: accuracy !== null && accuracy >= 50 },
        ].map(({ label: l, value, highlight }) => (
          <div key={l}>
            <div style={{ fontSize: 18, fontWeight: 700, color: highlight ? 'var(--success)' : 'inherit' }}>{value}</div>
...
function StatsCard({ stats }: { stats: HomeDashboard['predictionStats'] }) {
  return (
    <div className="card">
      <h3 style={{ margin: '0 0 14px', fontSize: 14, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--muted)' }}>
        My Prediction Stats
      </h3>
...
function UpcomingMatchRow({ match, last }: { match: UpcomingMatchWithPrediction; last: boolean }) {
  ...
  const predictionBadge = prediction?.isSubmitted ? (
    <span className="badge badge-info">
      Prediction:{' '}
      {prediction.predictedOutcome === 'HOME_WIN' ? `${match.homeTeam} Win` :
       prediction.predictedOutcome === 'AWAY_WIN' ? `${match.awayTeam} Win` : 'Draw'}
    </span>
  ) : prediction ? (
  ...
  return (
    <div style={{
      ...
      padding: '11px 16px', borderBottom: last ? 'none' : '1px solid var(--border)',
...
function UpcomingMatchesCard() {
  ...
  const [pendingByStage, setPendingByStage] = useState({ group: 0, knockout: 0 });
  const [page, setPage] = useState(0);
  const [loading, setLoading] = useState(true);
  const [paging, setPaging] = useState(false);

  useEffect(() => {
    getHomeDashboard(0).then((data) => {
  ...
  const ctaBadges = (
    <div style={{ display: 'flex', gap: 6 }}>
      {groupPending > 0 && (
        <Link href="/predictions?stage=GROUP" style={{ textDecoration: 'none' }}>
          <span className="badge badge-danger" style={{ cursor: 'pointer' }}>
            {groupPending} group
          </span>
        </Link>
      ...
      {koPending > 0 && (
        <Link href="/predictions?stage=KNOCKOUT" style={{ textDecoration: 'none' }}>
          <span className="badge badge-danger" style={{ cursor: 'pointer' }}>
            {koPending} knockout
          </span>
        </Link>
  ...
  return (
    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
        <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--muted)', whiteSpace: 'nowrap' }}>
          Upcoming Matches
        </h3>
      ...
      {loading ? (
        ...
        <>
          <div style={{ position: 'relative', opacity: paging ? 0.5 : 1, transition: 'opacity 0.15s' }}>
            {matches.map((match, i) => (
...
function Dashboard({ user }: { user: { username: string; avatarUrl: string | null; totalPoints: number } }) {
  const [stats, setStats] = useState<HomeDashboard['predictionStats'] | null>(null);

  useEffect(() => {
    getHomeDashboard(0).then((data) => setStats(data.predictionStats)).catch(() => {});
  }, []);
